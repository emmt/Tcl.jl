...
#
#     set A [::AffineTransforms2D::new]; # yields the identity
#     set A [::AffineTransforms2D::new $A0 $A1 $A2 $A3 $A4 $A5]
#
#
# Many operations are available to manage or apply affine transforms.  The most
# used is:
#
#     set xp_yp [::AffineTransforms2D::apply $A $x $y]
#
# yields a list of two transformed coordinates.  Two transforms can be
# combined:
#
#     set C [::AffineTransforms2D::combine $A $B]
#
# yields `C` which applies `B` then `A`.  Affine linear transforms can be
# translated:
#
#     set B [::AffineTransforms2D::leftTranslate $tx $ty $A]
#     set C [::AffineTransforms2D::rightTranslate $A $tx $ty]
#
...
# be rotated:
#
#     set B [::AffineTransforms2D::leftRotate $theta $A]
#     set C [::AffineTransforms2D::rightRotate $A $theta]
#
...
#
#     set B [::AffineTransforms2D::leftScale $rho $A]
#     set C [::AffineTransforms2D::rightScale $A $rho]
#
...
# divided:
#
#     set M [::AffineTransforms2D::invert $A]
#     set R [::AffineTransforms2D::leftDivide $A $B]
#     set S [::AffineTransforms2D::rightDivide $A $B]
#
# yield `M = A^{-1}`, `R = A\B` and `S = A/B`.
#
# The determinant and Jacobian can be computed by:
#
#    ::AffineTransforms2D::determinant $A
#    ::AffineTransforms2D::jacobian $A
#
# The coordinates of the intercept which is the point `(x,y)` such that
# `A*(x,y) = (0,0)` can be computed by:
#
#    ::AffineTransforms2D::intercept $A
#
namespace eval ::AffineTransforms2D {
    #
    ...
    #
    # `multiply $A $B` yields `A*B`, the affine transform which combines the
    # two affine transforms `A` and `B`, that is the affine transform which
    # applies `B` and then `A`.
    #
    proc multiply {A B} {
        set A0 [lindex $A 0]; set A1 [lindex $A 1]; set A2 [lindex $A 2]
        set A3 [lindex $A 3]; set A4 [lindex $A 4]; set A5 [lindex $A 5]
    ...
    #
    # `chain $A $B $C ...` yields `...*C*B*A`, the affine transform which
    # applies `A` then `B`, then `C`, etc.  Note that `chain $A $B` is the same
    # as `multiply $B $A` (order is reversed).
    #
    proc chain {A args} {
        foreach B $args {
            set A [multiply $B $A]
