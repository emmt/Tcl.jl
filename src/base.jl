...
@inline TclObj(str::AbstractString) =
    TclObj{String}(ccall((:Tcl_NewStringObj, libtcl), TclObjPtr,
                         (Ptr{UInt8}, Cint), str, sizeof(str)))

@inline TclObj(value::Symbol) = TclObj(string(value))

@inline TclObj(::Void) =
    TclObj{Void}(ccall((:Tcl_NewStringObj, libtcl), TclObjPtr,
                       (Ptr{UInt8}, Cint), NOTHING, 0))
...
Base.push!(lst::TclObj{List}, args...; kwds...) = lappend!(lst, args...; kwds...)
